...
from .directives import TimelineDirective
...
class TimelineDomain(Domain):
    ...
    _new_roles            : ClassVar[list[type]] = []
    _new_indices          : ClassVar[list[type]] = []

    initial_data : ClassVar[dict[str, dict]] = { # type: ignore[misc]
        'entries'       : {},
    ...
    def __init__(self, env:BuildEnvironment) -> None:
        ...
        # directives, roles, indices to be registered rather than in setup:
        self.directives   = {'timeline'        : TimelineDirective}
        self.indices        = TimelineDomain._new_indices[:]
        self.roles        = {'ref'          : XRefRole()}
        self.roles.update({x.reftype : x() for x in TimelineDomain._new_roles})
